...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	function addSongUI():Void
	{
		...
		var loadEventJson:FlxButton = new FlxButton(loadAutosaveBtn.x, loadAutosaveBtn.y + 30, 'Load Events', function()
		{
			...
			#if MODS_ALLOWED
			if (FileSystem.exists(Paths.modsJson(songName + '/events')) || #end FileSystem.exists(SUtil.getPath() + file))
			#else
		...
		var characters:Array<String> = CoolUtil.coolTextFile(SUtil.getPaths() + Paths.txt('characterList'));
		...
		#if MODS_ALLOWED
		var directories:Array<String> = [Paths.mods('stages/'), Paths.mods(Paths.currentModDirectory + '/stages/'), SUtil.getPaths() + Paths.getPreloadPath('stages/')];
		for(mod in Paths.getGlobalMods())
		...
		tempMap.clear();
		var stageFile:Array<String> = CoolUtil.coolTextFile(SUtil.getPaths() + Paths.txt('stageList'));
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput)
		{
			...
			if (FlxG.keys.pressed.W || FlxG.keys.pressed.S #if android || _virtualpad.buttonUp.Pressed #end)
			{
				FlxG.sound.music.pause();
			...
			if(!vortex){
				if (FlxG.keys.justPressed.UP || FlxG.keys.justPressed.DOWN #if android || _virtualpad.buttonUp.justPressed #end)
				{
					FlxG.sound.music.pause();
					updateCurStep();
	...
	*/
	...
	function loadHealthIconFromCharacter(char:String) {
		...
		#end
		{
			path = SUtil.getPaths() + Paths.getPreloadPath('characters/' + Character.DEFAULT_CHARACTER + '.json'); //If a character couldn't be found, change him to BF just to prevent a crash
		}
